...
public class FirstBadugiSubmission implements BadugiPlayer {
	...
	public FirstBadugiSubmission() {
		this.id = ++count;
	}

	public FirstBadugiSubmission(String nick) {
		this.nick = nick;
	}
	...
	public int bettingAction(int drawsRemaining, BadugiHand hand, int bets, int pot, int toCall, int opponentDrew) {
		...
		if(opponentDrew > 1 || raise){
			raise = true;
			return +1;
		}
		
		
		for(int i = 0; i<hand.getAllCards().size()-1; i++){
			if(hand.getAllCards().get(i).getRank() > 9){
				return -1;
			}
		}
